...
<div class="flex items-center justify-center min-h-screen bg-gray-100">
    <form action="{{ route('notes.update', $note->id) }}" method="POST" class="bg-white p-8 rounded shadow-md w-full max-w-lg">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="number" name="note" id="note" 
                   class="w-full border border-gray-300 p-2 rounded focus:outline-none focus:ring focus:ring-blue-300" 
                   min="0" max="20" step="0.25" value="{{ $note->note }}" required>
            @error('note')
                <div class="text-red-600 text-sm mt-2">{{ $message }}</div>
            @enderror
        </div>
